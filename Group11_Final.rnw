...
The TwitterToCorpus function will first remove any references to the original search term, if given. It will then covert the text to lower case. Next it removes any URLs. Then it removes any Twitter mentions. Numbers and stopwords are removed next, along with any punctuation. Finally, any non-aplha characters are replaces with a space and redundant whitespace is stripped from the data.
...
Now that the word counting of the corpus is complete. And the word matrix has been rolled up into a frequency data frame, we can display the most frequent words in the data set.
...
Next, we want to take a look at which words are most correlated with the top frequncy words in the data set. Using the NLP and tm packages, we can get a measure of word associations. What this means is that we will find out how commonly a word appears in the same tweet within our data set. Twitter data can yield some non-intuitive results.
...
p <- p + xlab('Association Level')
p <- p + ylab('Related Term')
